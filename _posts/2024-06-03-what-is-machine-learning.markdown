...
The $$a_i^0$$ elements represent the values generated by the activation function. For the input layer, these values are exactly the same as the input values (the independent variables).
...
If we then use our simple the neural network to compute our prediction (feed forward), we get:
